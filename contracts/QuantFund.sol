...
contract StandardToken is Context, Token {
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2) public virtual returns (bool success) {
        ...
        require(balances[_msgSender()] >= _value && _value > 0, "Transfer amount must be greater than zero");
...
contract QuantFund is StandardToken {
    ...
    function depositFrom(address participant) private {
        require(participant != address(0));
        require(block.number >= fundingStartBlock, "ICO is not started");
    ...
    function requestWithdrawal(uint256 amountTokensToWithdraw) external isTradeable {
        require(block.number > fundingEndBlock, "ICO is not ended");
        require(amountTokensToWithdraw > 0, "Withdrawal amount must be greater than zero");
        address participant = _msgSender();
        require(balanceOf(participant) >= amountTokensToWithdraw, "Balance must be grater than withdrawal amount");
